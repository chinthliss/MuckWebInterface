...
<template>
    <table class="table table-dark table-hover table-responsive">
        ...
        <tr>
            ...
            <th scope="col" colspan="2">Buy Price</th>
            <th scope="col" colspan="2">Sell Price</th>
            <th scope="col">Conversions</th><!-- Convert to reward tokens and upscale/downscale -->
        </tr>
        </thead>
        <tbody>
        <template v-for="type in types">
            <tr class="table-group-divider"><td colspan="7">
                <h3>{{ capital(type) }}</h3>
                ...
                <td>
                    <template v-if="type in config && rank in config[type]">
                        ...
                        <button v-if="config[type][rank].downscale"
                            ...
                            <i class="fas fa-down-long btn-icon-left"></i>Downscale {{ config[type][rank].downscale }}
                        </button>
                        ...
                        <button v-if="config[type][rank].tokens"
                            ...
                            <i class="fas fa-medal btn-icon-left"></i>Convert to {{ config[type][rank].tokens }} tokens
                        </button>
                    </template>
