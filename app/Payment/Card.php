...
class Card
{
    ...
    /**
     * @var string CardNumber - during capture this is the full number but after creation becomes the masked number
     */
     ...
     * @return bool
     */
    public static function checkLuhnChecksumIsValid(string|int $number): bool
    {
        $total = 0;
    ...
    public static function findIssuesWithAddCardParameters($cardNumber, $expiryDate, $securityCode): array
    {
        $errors = [];
        ...
        if (!preg_match('/^\d\d\/\d\d\d\d$/', $expiryDate)) {
            ...
            $endDate = Carbon::createFromDate((int)$year, (int)$month + 1, 1)->startOfDay();
