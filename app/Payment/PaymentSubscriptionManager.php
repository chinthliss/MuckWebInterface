...
class PaymentSubscriptionManager
{
    ...
    public function getSubscriptionsDuePayment(): array
    {
        ...
        foreach ($rows as $row) {
            $subscription = $this->buildSubscriptionFromRow($row);
            if ($subscription->nextChargeAt < Carbon::now()) $subscriptions[] = $subscription;
        }
     ...
     * @throws Exception
     */
    public function closeSubscription(PaymentSubscription $subscription, string $closureReason): void
    {
        ...
        if ($subscription->vendor === 'paypal') {
            abort(500, 'PayPalManager not reimplemented yet.');
            //TODO: Re-introduce PayPal cancellation flow after reimplementing PayPalManager
    ...
    public function suspendSubscription(PaymentSubscription $subscription): void
    {
     ...
     * @throws Exception
     */
    function processSubscription(PaymentSubscription $subscription): void
    {
     ...
     * @throws Exception
     */
    public function processPaidSubscriptionTransaction(PaymentSubscription $subscription, PaymentTransaction $transaction): void
    {
    ...
    public function updateVendorProfileId(PaymentSubscription $subscription, string $vendorProfileId): void
    {
        $subscription->vendorProfileId = $vendorProfileId;
        $this->storageTable()->where('id', '=', $subscription->id)->update([
    ...
    public function updateVendorSubscriptionId(PaymentSubscription $subscription, string $vendorSubscriptionId): void
    {
     ...
     * @param PaymentSubscription $subscription
     */
    public function setSubscriptionAsActive(PaymentSubscription $subscription): void
    {
