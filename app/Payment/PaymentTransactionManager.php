...
class PaymentTransactionManager
{
    ...
    public function __construct(MuckService $muck)
    {
        $this->muck = $muck;
    }
    ...
    private function insertTransactionIntoStorage(PaymentTransaction $transaction): void
    {
        $row = [
            'id' => $transaction->id,
    ...
    private function buildTransactionFromRow($row): ?PaymentTransaction
    {
        ...
        if ($row->items_json) {
            foreach (json_decode($row->items_json) as $itemArray) {
                $transaction->items[] = PaymentTransactionItem::fromArray($itemArray);
            }
        }
    ...
    public function findTransactions(string $vendor = null, string $vendorProfileId = null,
                                     string $vendorTransactionId = null, string $subscriptionId = null): array
    {
        $criteria = [];
        if ($vendor) $criteria[] = ['vendor', '=', $vendor];
        if ($vendorProfileId) $criteria[] = ['vendor_profile_id', '=', $vendorProfileId];
        if ($vendorTransactionId) $criteria[] = ['vendor_transaction_id', '=', $vendorTransactionId];
        if ($subscriptionId) $criteria[] = ['subscription_id', '=', $subscriptionId];
     ...
     * @throws Exception
     */
    public function closeTransaction(PaymentTransaction $transaction, string $closureReason): void
    {
     ...
     */
    public function setPaid(PaymentTransaction $transaction, bool $suppressNotification = false): void
    {
    ...
    public function updateVendorTransactionId(PaymentTransaction $transaction, string $vendorTransactionId): void
    {
    ...
    public function updateVendorProfileId(PaymentTransaction $transaction, string $vendorProfileId): void
    {
        $transaction->vendorProfileId = $vendorProfileId;
        $this->storageTable()->where('id', '=', $transaction->id)->update([
     ...
     * @throws Exception
     */
    public function chargeTransaction(PaymentTransaction $transaction): void
    {
            ...
            case 'authorizenet':
                ...
                try {
                    ...
                    if ($card) {
                        ...
                        Log::info("chargeTransaction - Unable to lookup card with ID $transaction->vendorProfileId for User {$user->id()} on Transaction $transaction->id.");
