...
class AuthorizeNetCardPaymentManager implements CardPaymentManager
{
     ...
     */
    private ?AnetAPI\MerchantAuthenticationType $merchantAuthentication = null;
    ...
    private function loadProfileFor(User $user): ?AuthorizeNetCardPaymentCustomerProfile
    {
        ...
        if ($row) {
            ...
            Log::debug('AuthorizeNet - Requesting profile#' . $profileId . ' for AID#' . $user->id());
     ...
     * @throws Exception
     */
    private function loadOrCreateProfileFor(User $user): AuthorizeNetCardPaymentCustomerProfile
    {
        $profile = $this->loadProfileFor($user);
        if (!$profile) {
            Log::debug('AuthorizeNet - Creating profile for User#' . $user->id());
            $anetProfile = new AnetAPI\CustomerProfileType();
            $anetProfile->setDescription("");
            $anetProfile->setMerchantCustomerId($user->id());
            ...
            if ($response && ($response->getMessages()->getResultCode() == "Ok")) {
                ...
                if ($attemptToFindExistingId && $attemptToFindExistingId[1]) {
                    LOG::warning('We have no valid Authorize.Net account for User#' . $user->id() .
                                  ...
                                  string $expiryDate, string $securityCode): Card
    {
        ...
        Log::debug('AuthorizeNet - Registering a new card for User#' . $user->id());
    ...
    {
        Log::debug("AuthorizeNet - Deleting card for " . $user->id() . ": " . $card->id);
        $profile = $this->loadProfileFor($user);
    ...
    {
        Log::debug("AuthorizeNet - Updating default card for " . $user->id() . " to " . $defaultCard->id);
        $profile = $this->loadOrCreateProfileFor($user);
        ...
        $request->setMerchantAuthentication($this->merchantAuthentication());
        $request->setCustomerProfileId($profile->getCustomerProfileId());
        $request->setCustomerPaymentProfileId($defaultCard->id);
        ...
        if (!$newPaymentProfileId) {
            Log::warning("AuthorizeNet - Set new default - "
                . "Our database isn't aware of the new payment profile id" . $defaultCard->id . "! Creating entry.");
            $newPaymentProfileId = $this->insertPaymentProfileIntoStorage($defaultCard,
                $profile->getCustomerProfileId(),
